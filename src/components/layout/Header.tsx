...
export default function Header({ locale }: { locale: L }) {
    ...
    const t = useMemo(() => {
        const dict: Record<L, { home: string; prices: string; contact: string; order: string; dial: string }> = {
    ...
    const linkBase =
        'px-3 py-2 text-sm font-medium transition hover:opacity-90';
    const active =
        'text-white';
    const inactive =
        'text-white/85';
    ...
    return (
        <header
            className={[
                ...
                'transition',
                scrolled ? 'backdrop-blur-md bg-slate-900/45 ring-1 ring-white/10' : 'bg-transparent',
            ].join(' ')}
        >
            <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
                ...
                <div className="hidden sm:flex items-center gap-2">
                    ...
                    <Link
                        href={`/${locale}/cennik`}
                        className={`${linkBase} ${pathname.startsWith(`/${locale}/cennik`) ? active : inactive}`}
                    >
                        {t.prices}
                    </Link>
                    <Link
                        href={`/${locale}/kontakt`}
                        className={`${linkBase} ${pathname.startsWith(`/${locale}/kontakt`) ? active : inactive}`}
                    >
                        {t.contact}
                    </Link>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                        ...
                        >
                            {locale.toUpperCase()} <svg width="14" height="14" viewBox="0 0 24 24"><path fill="currentColor" d="M7 10l5 5 5-5z"/></svg>
                        </button>
                        {openLang && (
                            <div
                                role="menu"
                            ...
                            >
                                {(['sk','en','de'] as L[]).map(l => (
                                    <button
                                        ...
                                        className={`block w-full px-3 py-2 text-left text-sm ${l===locale ? 'text-white' : 'text-white/80 hover:bg-white/10'}`}
                                    >
                                        {l.toUpperCase()}
                                    </button>
                                ))}
            ...
            {/* a subtle hairline under the header only when scrolled */}
            <div className={scrolled ? 'h-px w-full bg-white/10' : 'h-px w-full bg-transparent'} />
        </header>
    );
}
